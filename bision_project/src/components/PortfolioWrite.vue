  ...
  <v-dialog style="z-index:2000 !important; position:relative;"persistent v-model="dialog" max-width="800px">
    <v-card>
      <v-card-title>
        ...
        <v-btn v-if="step === 3" :disabled="!form" flat icon color="blue" @click="savePW">
          ...
          <v-layout wrap>
            ...
            <v-flex xs12 sm6>
              <v-menu ref="fromMenu" v-model="fromMenu" :close-on-content-click="false"
                ...
                <template v-slot:activator="{ on }">
                  <v-text-field color="blue" v-model="portfolio.fromDate" label="시작 날짜*" prepend-icon="event" readonly v-on="on"></v-text-field>
                </template>
            ...
            <v-flex xs12 sm6>
              <v-menu ref="toMenu" v-model="toMenu" :close-on-content-click="false"
                      ...
                      min-width="290px" offset-y full-width>
                <template v-slot:activator="{ on }">
                  <v-text-field color="blue" v-model="portfolio.toDate" label="종료 날짜*" prepend-icon="event" readonly v-on="on"></v-text-field>
                </template>
      ...
      <v-divider></v-divider>
      <v-card-actions>
          <v-btn icon flat color="warning" v-if="step !== 1"><v-icon large> keyboard_arrow_left </v-icon></v-btn>
          <v-spacer></v-spacer>
          <v-btn icon flat color="warning" v-if="step !== 3"><v-icon large> keyboard_arrow_right </v-icon></v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
