...
export default {
    ...
    mounted() {
        this.roomId=location.pathname.split('/')[2]
    ...
    methods: {
        ...
        getExistMessage(){
          this.$http.get('/api/room/findChatById/'+this.roomId)
            .then( res => {
              for (let i = res.data.length-1; i >=0; i--) {
                this.messages = [...this.messages, res.data[i]]
              }
            })
            .catch( err => {
              console.log(err)
        ...
        getRoomInfo(){
          this.$http.get('/api/room/findById/'+this.roomId)
          .then(res=>{
          ...
          if(this.getIsGuide){
            ...
            .then(res=>{
              ...
              for (var i = 0; i < res.data.length; i++) {
                if(res.data[i].user._id===this.roomInfo.user._id){
                  res.data.splice(i,1)
                }
              }
            ...
            .then(res=>{
              ...
              for (var i = 0; i < res.data.length; i++) {
                if(res.data[i].guide._id===this.roomInfo.guide._id){
                  res.data.splice(i,1)
                }
              }
              this.chatList=res.data;
